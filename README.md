...
<p align="center">
  <strong>一款 Windows 桌面 AI 语音输入工具。按热键说话，语音实时转文字注入光标。</strong>
</p>
...
听墨是一款 Windows 桌面 AI 语音输入工具。在任意应用中按下热键开始说话，松开后语音自动转文字注入光标位置——无需切换窗口、无需手动粘贴。

与主流 AI 输入法不同，听墨**不绑定厂商**：ASR（语音识别）和 LLM（大语言模型）都可以自由选择服务商。支持**完全离线运行**（本地 SenseVoiceSmall 模型），数据不上传云端。轻量托盘常驻，没有皮肤、没有弹窗、没有广告。
