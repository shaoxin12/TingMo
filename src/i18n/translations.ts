...
const D: Record<string, LocaleMap> = {
  ...
  'onboarding.cloudHint':        { 'zh-CN': '默认使用本地离线引擎。如需更高精度，可在 设置 → 模型 中切换到云端引擎并配置 API Key。', 'zh-TW': '默認使用本地離線引擎。如需更高精度，可在 設定 → 模型 中切換到雲端引擎並配置 API Key。', en: 'Defaulting to local offline engine. For higher accuracy, switch to cloud engine and configure API keys in Settings → Model.', ja: 'デフォルトはローカルオフラインエンジンです。より高い精度が必要な場合は、設定 → モデル でクラウドエンジンに切り替えてAPIキーを設定してください。', ko: '기본값은 로컬 오프라인 엔진입니다. 더 높은 정확도가 필요하면 설정 → 모델에서 클라우드 엔진으로 전환하고 API 키를 구성하세요.' },
